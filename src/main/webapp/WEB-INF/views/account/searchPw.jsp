...
<body>
	<div id="wrap" class="type02">
		...
		<div id="container">
			<div class="contents">
				<div class="inner-wrap">
					<strong class="stit">비밀번호 찾기</strong>
				</div>
				<form class="form-area" role="form" id="forms">
					<label><span class="form-tit">사용자 ID</span><input type="text"
						id="user_id" name="user_id" placeholder="사용자 ID"></label> <label><span
						class="form-tit">사용자명</span><input type="text" id="user_name"
						name="user_name" placeholder="사용자명"></label> <label><span
						class="form-tit">휴대폰번호</span><input type="text" class="form-tit"
						id="phone_no" name="phone_no" placeholder="전화번호('-'없이)"></label>
						 <label><span
						class="form-tit">이메일주소</span><input type="text" class="form-tit"
						id="email" name="email" placeholder="비밀번호 받을 E-Mail">
					<!-- <a href="#" class="btn-nor">본인인증</a>--></label>
		...
		<div class="progress-btn-area">
			<div class="inner-wrap">
				...
				<a href="#" class="next" onclick="search_pw()">비밀번호 찾기</a>
			</div>
		</div>
	</div>
	...
	<script>
		function search_pw() {
			...
			if (null == email || "" == email || null == user_id || "" == user_id || null == user_name || "" == user_name || null == phone_no
					|| "" == phone_no) {
				alert("정보를 입력해주세요.");
			} else {
				$
						.ajax({
							...
							success : function(data) {
								if (data == "no data") {
									...
									window.location.href = '/account/searchPwResult';
								}
							},
